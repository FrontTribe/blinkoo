...
export default function StaffRedeemPage() {
  const t = useTranslations('staffRedeem')
  ...
  async function handleRedeem() {
    ...
    try {
      ...
      if (response.ok) {
        setResult({ type: 'success', message: t('messages.success') })
        setCode('')
        // Clear result after 3 seconds
        setTimeout(() => setResult(null), 3000)
      } else {
        setResult({ type: 'error', message: data.error || t('messages.failed') })
      }
    } catch (error) {
      setResult({ type: 'error', message: t('messages.failedDefault') })
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-2xl mx-auto space-y-6 md:space-y-8">
        ...
        <div>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm md:text-base text-text-secondary">{t('subtitle')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-border rounded-lg p-6 md:p-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-text-secondary text-center">{t('codeHint')}</p>
          </div>
          ...
          <div className="bg-white border border-border rounded-lg p-4 hover:border-primary transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-text-primary">{t('scanRedeem')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-border rounded-lg p-4 hover:border-primary transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-text-primary">{t('handsFree')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-border rounded-lg p-4 hover:border-primary transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-text-primary">{t('verified')}</p>
              </div>
            </div>
          </div>
        </div>
