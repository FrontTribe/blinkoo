...
export function VoiceInputButton({ onResult, onCancel }: VoiceInputButtonProps) {
  const t = useTranslations('staffRedeem.voiceInput')
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg p-6 max-w-sm w-full">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-text-primary">{t('listening')}</h3>
            <button
              onClick={() => {
                stopListening()
                onCancel()
          ...
          {transcript && (
            <p className="text-center text-sm text-text-secondary mb-4">
              {t('heard')} <span className="font-mono font-semibold">{transcript}</span>
            </p>
          )}
          <p className="text-xs text-center text-text-tertiary">{t('instructions')}</p>
        </div>
      </div>
    )
  }
