...
export default function ClaimsManagementPage() {
  const t = useTranslations('merchant.claims')
  ...
  async function handleBulkAction(action: string) {
    if (selectedClaims.size === 0) {
      toast.error(t('toast.noClaimsSelected'))
      return
    }
    ...
    try {
      ...
      const data = await response.json()
      toast.success(t('toast.bulkUpdateSuccess', { count: data.updated }))
      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="bg-white border border-border rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-heading text-lg font-bold text-text-primary">{t('filters.title')}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="RESERVED">{t('summary.reserved')}</option>
                <option value="REDEEMED">{t('summary.redeemed')}</option>
                <option value="EXPIRED">{t('summary.expired')}</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-text-primary mb-2">
        ...
        {selectedClaims.size > 0 && (
          <div className="bg-primary/10 border border-primary rounded-lg p-4 flex items-center justify-between">
            <span className="text-sm font-semibold text-text-primary">
              {t('bulkActions.selected', { count: selectedClaims.size })}
            </span>
        ...
        <div className="bg-white border border-border rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-border bg-gradient-to-r from-primary/5 to-transparent">
            <h2 className="font-heading text-lg font-bold text-text-primary">
              {t('table.title', { count: claims.length })}
            </h2>
          </div>
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-text-tertiary font-medium">{t('table.noClaimsFound')}</p>
                        <p className="text-text-tertiary text-sm mt-1">{t('table.tryAdjustingFilters')}</p>
                      </div>
                    </td>
                  </tr>
                ) : (
            ...
            <div className="bg-white rounded-lg max-w-md w-full p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-heading text-xl font-bold text-text-primary">
                  {t('qrModal.title', { id: String(selectedClaim.id).slice(-8) })}
                </h3>
                <button
